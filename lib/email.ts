...
export const sendWelcomeEmail = async (email: string, name?: string) => {
    try {
        const { data, error } = await resend.emails.send({
            from: 'AI Portfolio <onboarding@resend.dev>', // Use resend.dev for testing if no domain
            to: [email],
            subject: 'Benvenuto in AI Portfolio!',
            html: `
                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                    <h1>Benvenuto ${name || ''}! 🚀</h1>
                    <p>Grazie per esserti iscritto a AI Portfolio. Siamo felici di averti a bordo.</p>
                    ...
                    <p>A presto,<br/>Il team di AI Portfolio</p>
                </div>
            `,
        });
...
export const sendNewsletter = async (email: string, subject: string, content: string) => {
    try {
        const { data, error } = await resend.emails.send({
            from: 'AI Portfolio Newsletter <newsletter@resend.dev>',
            to: [email],
            subject: subject,
            html: `
                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <p style="font-size: 12px; color: #666;">
                        Hai ricevuto questa mail perché sei iscritto alla newsletter di AI Portfolio.
                    </p>
                </div>
            `,
        });
